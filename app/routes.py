...
server_connection = Connectserver(host='127.0.0.1', user='root', password='', port='3306')  # conecta com o banco de dados (já criado no arquivo createdb.py)

login = False
produtos_carrinho = {}
nomes = []
names = server_connection.select('hometreasures', 'PRODUTOS', ['NOME'])
for result in names:
...
def process_login():
    ...
    if request.method == 'POST':
        ...
        result = server_connection.select('hometreasures', 'CLIENTES', ['EMAIL_LOGIN', 'SENHA', 'NOME'], f""" EMAIL_LOGIN = '{inserted_email}' """)  # lista de tuplas com os registros que têm esse email
...
def process_register():
    ...
    if request.method == 'POST':
        ...
        if password != confirm_password:
            ...
            result_email = server_connection.select('hometreasures', 'CLIENTES', '*', f""" EMAIL_LOGIN = '{login_email}' """)
            result_cpf = server_connection.select('hometreasures', 'CLIENTES', '*', f""" CPF = '{cpf}' """)
            ...
            if len(result_email) > 0:
                ...
                result = server_connection.insert('hometreasures', 'CLIENTES', ['NOME', 'DATA_NASC', 'CPF', 'GENERO', 'ESTADO_CIVIL', 'NACIONALIDADE', 'EMAIL_CONTATO', 'TELEFONE', 'CEP', 'ESTADO', 'CIDADE', 'LOGRADOURO', 'COMPLEMENTO', 'EMAIL_LOGIN', 'SENHA'], (f'{name}', f'{data_nasc}', f'{cpf}', f'{gender}', f'{estado_civil}', f'{nacionality}', f'{contact_email}', phone, f'{cep}', f'{state}', f'{city}', f'{logradouro}', f'{complement}', f'{login_email}', f'{password}'))     # adiciona o novo cadastro ao banco de dados
                if result[0]:
...
def info_account():
    ...
    aux_account_info = server_connection.select('hometreasures', 'CLIENTES', ['NOME', 'DATA_NASC', 'CPF', 'GENERO', 'ESTADO_CIVIL', 'NACIONALIDADE', 'EMAIL_CONTATO', 'TELEFONE', 'CEP', 'ESTADO', 'CIDADE', 'LOGRADOURO', 'COMPLEMENTO', 'EMAIL_LOGIN', 'SENHA'], f""" EMAIL_LOGIN = '{email_login}' """)[0]
...
def update_account():
    ...
    if request.method == 'POST':
        ...
        if nova_senha != confirmar_nova_senha:
            ...
            if nova_senha == '':
                senha_antiga = server_connection.select('hometreasures', 'CLIENTES', ['SENHA'], f""" EMAIL_LOGIN = '{email_login}' """)[0][0]
                nova_senha = senha_antiga

            result = server_connection.update('hometreasures', 'CLIENTES', ['NOME', 'EMAIL_CONTATO', 'TELEFONE', 'CEP', 'NACIONALIDADE', 'ESTADO', 'CIDADE', 'LOGRADOURO', 'COMPLEMENTO', 'ESTADO_CIVIL', 'SENHA'], [f'{nome}', f'{email_contato}', f'{telefone}', f'{cep}', f'{nacionalidade}', f'{estado}', f'{cidade}', f'{logradouro}', f'{complemento}', f'{estado_civil}', f'{nova_senha}'], f""" EMAIL_LOGIN = '{email_login}' """)
            if result[0]:
...
def carrinho():
    ...
    for product_name, amount in produtos_carrinho.items():
        if amount != 0:
            ...
            product_price = server_connection.select('hometreasures', 'PRODUTOS', ['PRECO'], f""" NOME = '{product_name}' """)[0][0]
...
def obter_infos_produtos():
    ...
    for cod in range(1, 5):
        result = server_connection.select('hometreasures', 'PRODUTOS', ['NOME', 'DESCRICAO', 'LINK_IMAGEM', 'PRECO'], f'COD_PRODUTO = {cod}')
        nome = result[0][0]
...
def obter_link_produto(produto):
    ...
    result = server_connection.select('hometreasures', 'PRODUTOS', ['LINK_IMAGEM'], f""" NOME = '{produto}' """)
    link = result[0][0]
    return link
